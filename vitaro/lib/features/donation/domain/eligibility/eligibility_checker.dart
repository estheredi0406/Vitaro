...
class EligibilityChecker {
  ...
  EligibilityResult evaluate(DonorProfile profile, {DateTime? referenceDate}) {
    ...
    if (profile.bloodPressureSystolic < 90 ||
        profile.bloodPressureSystolic > 150) {
      reasons.add('Systolic blood pressure must be between 90 and 150 mmHg.');
    }

    if (profile.bloodPressureDiastolic < 60 ||
        profile.bloodPressureDiastolic > 100) {
    ...
    if (profile.lastDonationDate != null) {
      ...
      if (daysSinceLastDonation < minimumDaysBetweenDonations) {
        final calculatedDate = lastDonationDate.add(
          Duration(days: minimumDaysBetweenDonations),
        );
    ...
    if (profile.nextEligibilityDate != null &&
      ...
      reasons.add('Next eligible date is ${_formatDate(upcomingDate)}.');
    }
